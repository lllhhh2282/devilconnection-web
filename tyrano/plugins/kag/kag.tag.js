...
tyrano.plugin.kag.tag.image = {
  ...
  start: function (pm) {
    ...
    // 画像を preload してから表示することで、Web 環境でのちらつき/空白を防ぐ
    this.kag.preload(strage_url, function () {
      if ('base' != pm.layer) {
        var img_obj = $('<img />')
        ...
        if ('' != pm.time) {
          ...
          'back' == pm.depth
            ? that.kag.layer.getLayer(pm.layer, pm.page).prepend(img_obj)
            : that.kag.layer.getLayer(pm.layer, pm.page).append(img_obj)
          img_obj
            .stop(!0, !0)
            .animate(
          ...
          'back' == pm.depth
            ? that.kag.layer.getLayer(pm.layer, pm.page).prepend(img_obj)
            : that.kag.layer.getLayer(pm.layer, pm.page).append(img_obj)
          !pm.stop && that.kag.ftag.nextOrder()
        }
      } else {
        var new_style = {
        ...
        that.kag.setStyles(
          that.kag.layer.getLayer(pm.layer, pm.page),
          new_style
        )
        !pm.stop && that.kag.ftag.nextOrder()
      }
