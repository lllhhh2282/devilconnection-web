tyrano.plugin.kag = {
  ...
  init_game: function () {
    ...
    // デバッグメニューの常時表示ボタン状態を反映
    if (TYRANO.updateGlobalDebugButton) {
      TYRANO.updateGlobalDebugButton()
    }
  ...
  loadScenario: function (file_name, call_back) {
    ...
    if (that.cache_scenario[file_url]) {
      ...
      that.prefetchScenarios(tag_obj)
    } else
  ...
  preload: function (src, callbk, priority) {
    var that = this,
      ext = $.getExt(src)
    if ('mp3' == ext || 'ogg' == ext || 'm4a' == ext || 'wav' == ext) {
      0 !== src.indexOf('http://') &&
        0 !== src.indexOf('https://') &&
        0 !== src.indexOf('./') &&
        (src = './' + src)
      // 既にキャッシュにあればそれを返す
      if (this.kag.tmp.howl_cache[src]) {
        callbk && callbk(this.kag.tmp.howl_cache[src])
        return
      }
      $.loadQueue(src, priority || 0).then(
        function (howl) {
          that.kag.tmp.howl_cache[src] = howl
          callbk && callbk(howl)
        },
        function () {
          callbk && callbk()
  ...
  preloadAll: function (storage, callbk, priority) {
