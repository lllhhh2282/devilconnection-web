tyrano.plugin.kag.tag.playbgm = {
  ...
  play: function (pm) {
    ...
    var audio_url = $.isHTTP(pm.storage)
      ? storage
      : '' != storage
      ? './data/' + target + '/' + storage
      : ''
    ...
    // Howl キャッシュを活用：同じ URL は再利用して再ロード/再デコードを回避
    if (
      audio_url &&
      this.kag.tmp.howl_cache[audio_url] &&
      this.kag.tmp.howl_cache[audio_url]._src
    ) {
      audio_obj = this.kag.tmp.howl_cache[audio_url]
      audio_obj.volume(volume)
      audio_obj.loop($.toBoolean(pm.loop))
    } else {
      audio_obj = new Howl(howl_opt)
      if (audio_url) {
        this.kag.tmp.howl_cache[audio_url] = audio_obj
      }
    }

    if ('bgm' === target) {
      // 同じ URL の場合は unload しない（キャッシュ保持）
      if (
        this.kag.tmp.map_bgm[pm.buf] &&
        this.kag.tmp.map_bgm[pm.buf]._src !== audio_obj._src
      ) {
        this.kag.tmp.map_bgm[pm.buf].stop()
      }
      ...
      if (this.kag.tmp.map_se[pm.buf]) {
        this.kag.tmp.map_se[pm.buf].stop()
      }
