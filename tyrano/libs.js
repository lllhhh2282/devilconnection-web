;(function ($) {
  ...
  //複数のスクリプトを一括して読み込み（並列ダウンロード、順序通り実行）
  $.getMultiScripts = function (arr, cb) {
    ...
    var results = new Array(cnt_script),
      loaded = 0

    function loadOne(index) {
      $.getScript(arr[index], function () {
        results[index] = true
        loaded++
        if (loaded == cnt_script) {
          // $.getScript がダウンロードと実行を同時に行うため、完了後に cb を呼ぶだけ
          if (typeof cb == 'function') {
            cb()
          }
        }
    ...
    for (var i = 0; i < cnt_script; i++) {
      loadOne(i)
    }
  }

  // リソース読み込みキュー：並列数制限 + 同一URLの重複排除
  $.loadQueue = (function () {
    var concurrency =
      (window.TYRANO && window.TYRANO.resource_concurrency) || 6
    var running = 0
    var queues = {
      image: [],
      audio: [],
    ...
    var inflight = {} // url -> { promise, callbacks: [] }

    function typeFromUrl(url) {
      var ext = url.split('.').pop().toLowerCase()
      if (/^(png|jpg|jpeg|gif|webp|svg|bmp|ico)$/.test(ext)) return 'image'
      if (/^(mp3|ogg|m4a|wav|aac|flac)$/.test(ext)) return 'audio'
      if (/^(mp4|ogv|webm|mov)$/.test(ext)) return 'video'
      return 'other'
    }
    ...
    function dispatch(type) {
      ...
      while (running < concurrency && q.length > 0) {
        var task = q.shift()
        running++
        notifyStart(task.url)
        var loader
        switch (type) {
          case 'image':
            loader = loadImage(task.url)
            break
          case 'audio':
            loader = loadAudio(task.url)
            break
          case 'video':
            loader = loadVideo(task.url)
            break
          default:
            loader = loadImage(task.url)
        }
        loader
          .then(function (result) {
            notifyComplete(task.url)
            task.resolve(result)
          })
          .catch(function (err) {
            notifyError(task.url)
            task.reject(err)
          })
          .finally(function () {
            running--
    ...
    function enqueue(url, priority) {
      ...
      var p = new Promise(function (resolve, reject) {
        ...
        if (priority > 0) {
          queues[type].unshift(task)
        } else {
          queues[type].push(task)
        }
        dispatch(type)
      })
      var record = { promise: p, callbacks: [] }
      inflight[url] = record
      p.then(
        function (result) {
          delete inflight[url]
    ...
    enqueue.setConcurrency = function (n) {
      concurrency = n
    }
  ...
  $.loadText = function (file_path, callback) {
    // ブラウザ本番環境ではテキストをキャッシュ可能にする（ローカル開発時は無効化）
    var cache_enabled =
      window.TYRANO && window.TYRANO.cache_text !== undefined
        ? window.TYRANO.cache_text
        : !/^(localhost|127\.0\.0\.1|\[::1\])$/.test(location.hostname)
    var url = cache_enabled ? file_path : file_path + '?' + Math.floor(Math.random() * 1000000)
  ...
  $.alert = function (str, cb, buttonText = 'OK') {
    if (window.USE_SWEETALERT2 && typeof Swal !== 'undefined') {
      Swal.fire({
        text: str,
